...
| 4 | **3D-spårbarhet** | Bygger förtroende — varje rad i inköpsunderlaget kan verifieras visuellt mot den riktiga 3D-modellen, inte bara siffror i en tabell. | Frontend laddar `772_H811_new.ifc` (web-ifc + three.js) och bygger en `expressID ↔ Tag`-karta. Klick på en rad i inköpsunderlaget slår upp raspektive `oid` mot `Tag` och highlightar elementet i 3D-vyn. | **Test:** givet känt OID `589830`, mappningen hittar rätt `expressID` i den laddade IFC-modellen. **Klick:** klicka raden för OID 589830, se balken "FD5 Opening header beam" lysa upp i 3D-vyn. | **Klar** |

## Status

Alla fyra inkrementen är byggda och verifierade mot de riktiga filerna:

| # | Automattest | Utfall |
|---|---|---|
| 1 | `pytest` (`backend/tests/test_xml_parser.py`, `test_bom.py`) | 731 rader ur `components.xml`, 72 med tvärsnitt `45x182` |
| 2 | `pytest` (`backend/tests/test_bom_groups.py`) | 24 grupper; `45x182/C24` har 72 bitar och handelslängderna 1800–5400 mm ur CSV:n |
| 3 | `pytest` (`backend/tests/test_cutting_optimizer.py`, `test_purchase_order.py`, `test_csv_export.py`) | Materialet balanserar på alla 525 stänger; spill **3,90 %** mot **15,21 %** ooptimerat; alla 50 överlånga bitar skarvade |
| 4 | `npm run verify:ifc` i `frontend/` | 731 IFC-element med `Tag`; OID `589830` → expressID 1554 = "FD5 Opening header beam 45x182 C24"; 0 BOM-rader utan geometri |

Klick-kontrollerna i tabellen ovan görs i appen (`npm run dev` + backend igång).
